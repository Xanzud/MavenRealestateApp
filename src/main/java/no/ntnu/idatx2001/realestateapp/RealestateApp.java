...
public class RealestateApp {
    ...
    public void addRealestatToRegister() {
        ...
        //if (reader.hasNextInt() && reader.nextInt() >= 0101 && reader.nextInt() <= 5444) {
        if (reader.hasNextInt()){
            int tempInt = reader.nextInt();
            if(tempInt >= 0101 && reader.nextInt() <= 5444) {
                municipalityNumber = tempInt;
            }
            reader.nextLine();
        } else {
            System.out.println("The Municipality number must be a number from 0101 to 5444");
    ...
    private void findRealestateByOwner() {
        ...
        if (owner.isBlank() || owner.isEmpty()) {
            ...
            Iterator<Property> propertiesFound = properties.findAllPropertiesByOwner(owner);
            Boolean noFoundProperties = true;
            while(propertiesFound.hasNext())
            {
                Property property = propertiesFound.next();
                displayProperty(property);
                noFoundProperties = false;
            }
            if(noFoundProperties == true)
            {
                System.out.println("No properties found with that owner.");
            }
        }
